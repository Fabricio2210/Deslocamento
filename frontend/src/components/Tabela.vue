  ...
  <div class="container">
    ...
    <div class="form-group py-2">
      ...
      >
        <button
          key="um"
          class="btn btn-secondary btn-lg btn-block list-complete-item"
          v-if="idDoc && !isDeslocamento"
          v-on:click="enviaDeslocamento()"
        >
          Gerar Deslocamentos
        </button>
        <button
          key="dois"
          class="btn btn-warning btn-lg btn-block list-complete-item"
          v-if="isDeslocamento && isbtnYelo"
          v-on:click="teste()"
        >
      ...
      >
        <a
          key="quatro"
          :href="`seu backend aqui/uploads/${idDoc}.xlsx`"
          role="button"
          class="btn btn-success btn-lg btn-block list-complete-item"
          v-if="isReset"
          >Baixar Arquivo</a
        >
      </transition>
    ...
    <div class="row" v-if="isDeslocamento">
      ...
      <div id="deslocamento" class="col-sm-10">
        <li class="list-group-item active text-center">
          <a class="btn btn-primary" v-on:click="selectText('deslocamento')"
            >Deslocamento <v-icon name="copy"
          /></a>
        </li>
        <li class="list-group-item" v-for="data in info" v-bind:key="data.id">
          {{ data.DESLOCAMENTO }}
        </li>
      </div>
      <div id="km" class="col-sm-2">
        <li class="list-group-item active">
          <a class="btn btn-primary" v-on:click="selectText('km')">
            KM <v-icon name="copy"
          /></a>
        </li>
        <li
          class="list-group-item"
          v-for="(desloc, index) in deslocamento"
          v-bind:key="`desloc-${index}`"
        >
          {{ desloc }}
        </li>
      </div>
      ...
      axios
        ...
        .then(res => {
          ...
          this.$notify({
            group: "sucesso",
            text:
              '<h6 class="text-center">Deslocamentos calculados com sucesso!</h6>',
            type: "success",
          });
        })
        .catch(erro => {
